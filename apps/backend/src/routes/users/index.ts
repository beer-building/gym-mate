...
const usersRoute: FastifyPluginAsync = async (server) => {
    ...
    async (request, reply) => {
      ...
      if (existUser)
        throw server.httpErrors.badRequest(
          usersErrors.ALREADY_REGISTERED_ERROR
        );
  ...
  server.post<LoginRequest>(
    ...
    async (request, reply) => {
      ...
      if (!existUser)
        throw server.httpErrors.unauthorized(usersErrors.USER_NOT_FOUND_ERROR);

      const isCorrectPass = await compare(password, existUser.passwordHash);

      if (!isCorrectPass)
        throw server.httpErrors.unauthorized(usersErrors.WRONG_PASS_ERROR);
